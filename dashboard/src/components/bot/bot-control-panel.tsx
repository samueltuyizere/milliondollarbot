...
export function BotControlPanel({ active = true }: BotControlPanelProps) {
  ...
  const fmt = (v: number | null | undefined, prefix = "$", decimals = 0) =>
    v != null
      ? `${prefix}${Math.abs(v).toLocaleString("en-US", {
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          tone={
            (data?.drawdownPct ?? 0) > 3
              ? "loss"
              : (data?.drawdownPct ?? 0) > 1.5
              ? "warning"
              : "neutral"
          }
        />
        <StatCard
          ...
          sub={
            data?.lastPing
              ? new Date(data.lastPing).toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                  second: "2-digit",
                })
      ...
      <div className="rounded-lg border border-border bg-card overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-3 gap-2">
            ...
            >
              <Play
                className={cn("w-5 h-5", s === "RUNNING" ? "text-[--profit]" : "")}
              />
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            <strong className="text-foreground">Stop</strong> sends a graceful shutdown.
            After a <strong className="text-foreground">Daily Lock</strong>, only a manual
            Start can resume trading.
          </p>
        </div>
      </div>
    </div>
